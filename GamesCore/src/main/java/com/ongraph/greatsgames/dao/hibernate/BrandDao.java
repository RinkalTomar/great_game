...
public class BrandDao extends AbstractDao<Brand> {
    ...
    public int count(BrandSearchCriteria searchCriteria) {
        int totalRows = 0;
    ...
    public List<Brand> search(BrandSearchCriteria searchCriteria, boolean isExportRequest) {
        TypedQuery<Brand> query = getSearchQuery(searchCriteria, false, Brand.class);
       /* if (!isExportRequest) {
    ...
    public <T> TypedQuery<T> getSearchQuery(BrandSearchCriteria searchCriteria, boolean isCountQuery,  Class<T> type) {
        // standard fields
